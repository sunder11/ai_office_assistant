# Let me create and display the modified code file
import os
...
def load_document(file_path):
    ...
    if not documents:
        raise ValueError(f"Could not extract text from DOCX file: {file_path}")
    return documents


def load_documents_from_folder(folder_path):
    """Load all DOCX documents from a folder"""
    all_documents = []
    docx_files = []
    ...
    if not docx_files:
        raise ValueError(f"No DOCX files found in folder: {folder_path}")
    ...
    return all_documents, docx_files
...
upload_option = st.radio(
    "Choose upload option:", ["Upload single DOCX", "Process folder of DOCX"]
)
if upload_option == "Upload single DOCX":
    uploaded_file = st.file_uploader(label="Upload your DOCX file", type=["docx"])
    if uploaded_file:
        ...
        if uploaded_file.name not in st.session_state.loaded_files:
            ...
            try:
                with st.spinner("Adding DOCX to existing collection..."):
                    documents = load_document(file_path)
                    st.session_state.vectorstore.add_documents(documents)
                    st.session_state.loaded_files.append(uploaded_file.name)
                    st.success(f"Added {uploaded_file.name} to collection")
            except Exception as e:
                st.error(f"Error adding DOCX: {str(e)}")
else:  # Process folder of DOCXs
    col1, col2 = st.columns([3, 1])
    with col1:
        folder_path = st.text_input(
            "Enter folder path containing DOCXs:",
            placeholder="e.g., /mnt/c/docxs or /home/user/docxs",
        )
    with col2:
    ...
    if process_button and folder_path:
        ...
        if not os.path.exists(folder_path):
            ...
            try:
                with st.spinner("Processing DOCXs in folder..."):
                    documents, docx_files = load_documents_from_folder(folder_path)
                    st.success(
                        f"Successfully loaded {len(documents)} document(s) from {len(docx_files)} DOCX files"
                    )
                    # Show processed files
                    with st.expander("Processed files", expanded=True):
                        for docx_file in docx_files:
                            st.write(f"{os.path.basename(docx_file)}")
                    ...
                    st.session_state.loaded_files.extend(
                        [os.path.basename(f) for f in docx_files]
                    )
            except Exception as e:
...
if "vectorstore" in st.session_state:
    ...
    st.info("Please upload a DOCX file or select a folder to start chatting")
