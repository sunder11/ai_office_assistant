...
upload_option = st.radio(
    ...
    ["Upload single PDF", "Process folder of PDFs"],
    horizontal=True,
)

if upload_option == "Upload single PDF":
    ...
    if uploaded_file:
        ...
        if uploaded_file.name not in st.session_state.loaded_files:
            if "vectorstore" not in st.session_state:
                ...
                try:
                    with st.spinner("Adding PDF to existing collection..."):
                        documents = load_document(file_path)
...
else:  # Process folder of PDFs
    col1, col2 = st.columns([3, 1])
    with col1:
        folder_path = st.text_input(
            "Enter folder path containing PDFs:",
            placeholder="e.g., /mnt/c/pdfs or /home/user/pdfs",
        )
    with col2:
    ...
    if process_button and folder_path:
        ...
        if not os.path.exists(folder_path):
            ...
            try:
                with st.spinner("Processing PDFs in folder..."):
                    documents, pdf_files = load_documents_from_folder(folder_path)
                    st.success(
                        f"Successfully loaded {len(documents)} document(s) from {len(pdf_files)} PDF files"
                    )
...
if "vectorstore" in st.session_state:
    ...
    st.info("👆 Please upload a PDF file or select a folder to start chatting")
